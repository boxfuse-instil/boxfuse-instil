(function (window) {
    ...
    var initTOC = function (options) {
        ...
        if (!selector.match(/^(?:h[1-6],?\s*)+$/g)) {
            throw new TypeError('selector must contain only h1-6');
        }
...
var mobileTOC = document.querySelector('.toc-toggle');
var mobileTOCLinks = document.querySelectorAll('.toc-nav a');

mobileTOC.addEventListener('click', function tocToggle(){
  mobileTOC.classList.toggle('toc-toggle--active');
});

mobileTOCLinks.forEach( element => {
  element.addEventListener('click', function tocToggle(){
    mobileTOC.classList.remove('toc-toggle--active');
  });
});

console.log(mobileTOCLinks);